...
- 暴露 cordova sqlite 插件的关闭连接接口
- 添加 prefetch、filter 支持带点的对象图写法，支持关联查询
```
Department.all().prefetch('parent').prefetch('employees')
    .filter('parent.name','=',name)
    .list(null,callback);
```
- 修正 or 和 and 复杂查询 hasOne、hasMany 生成 SQL 不正确的问题
