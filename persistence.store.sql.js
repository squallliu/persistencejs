...
function config(persistence, dialect) {
  ...
  persistence.NullFilter.prototype.sql = function (meta, alias, values, qc) {
    return "1=1";
  };

  persistence.AndFilter.prototype.sql = function (meta, alias, values, qc) {
    return "(" + this.left.sql(meta, alias, values, qc) + " AND "
      + this.right.sql(meta, alias, values, qc) + ")";
  };

  persistence.OrFilter.prototype.sql = function (meta, alias, values, qc) {
    return "(" + this.left.sql(meta, alias, values, qc) + " OR "
      + this.right.sql(meta, alias, values, qc) + ")";
  };

  persistence.PropertyFilter.prototype.sql = function (meta, alias, values, qc) {
    var tm = persistence.typeMapper;
    var splittedProp = this.property.split('.');
    var cjPropFilter = splittedProp.length > 1;
    var aliasPrefix, sqlType;
    var sqlProperty = this.property;
    if (cjPropFilter) {
      aliasPrefix = "";
      for (var cm in qc._calculatedJoins) {
        if (qc._calculatedJoins[cm].tableAlias == splittedProp[0]) {
          sqlType = qc._calculatedJoins[cm].meta.fields[splittedProp[1]];
          break;
        }
      }
      sqlProperty = splittedProp[0] + '`.`' + splittedProp[1];
    } else {
      aliasPrefix = alias ? "`" + alias + "`." : "";
      sqlType = meta.fields[this.property] || tm.idType;
    }
    if (this.operator === '=' && this.value === null) {
      return aliasPrefix + '`' + sqlProperty + "` IS NULL";
    } else if (this.operator === '!=' && this.value === null) {
      return aliasPrefix + '`' + sqlProperty + "` IS NOT NULL";
      ...
      if (vals.length === 0) {
        ...
        return aliasPrefix + '`' + sqlProperty + "` IN (" + qs.join(', ') + ")";
      }
      ...
      if (vals.length === 0) {
        ...
        return aliasPrefix + '`' + sqlProperty + "` NOT IN (" + qs.join(', ') + ")";
      }
    } else if (this.operator === 'like' || this.operator === 'not like') {
      var result = aliasPrefix + '`' + sqlProperty + '` ' + this.operator.toUpperCase() + ' ' + tm.outVar('?', sqlType);
      ...
      return aliasPrefix + '`' + sqlProperty + "` " + this.operator + " " + tm.outVar("?", sqlType);
    }
  };
  ...
  persistence.DbQueryCollection.prototype.list = function (tx, callback) {
    ...
    var currentMeta, calculatedJoinSql = '';
    for (var currentCj in this._calculatedJoins) {
      currentMeta = this._calculatedJoins[currentCj];
      if (meta.hasMany.hasOwnProperty(currentMeta.refProperty)) {
        var temp = meta.hasMany[currentMeta.refProperty].inverseProperty;
        calculatedJoinSql += "LEFT JOIN `" + currentMeta.meta.name + "` AS `" + currentMeta.tableAlias
          + "` ON `" + currentMeta.tableAlias + "`.`" + temp + "` = `" + currentMeta.refTableAlias + "`.`id` ";
      } else {
        calculatedJoinSql += "LEFT JOIN `" + currentMeta.meta.name + "` AS `" + currentMeta.tableAlias
          + "` ON `" + currentMeta.tableAlias + "`.`id` = `" + currentMeta.refTableAlias + '`.`' + currentMeta.refProperty + "` ";
      }
    }
    joinSql += calculatedJoinSql;

    var whereSql = "WHERE "
      + [this._filter.sql(meta, mainAlias, args, this)].concat(additionalWhereSqls).join(' AND ');
  ...
  persistence.DbQueryCollection.prototype.destroyAll = function (tx, callback) {
    ...
    var args = [];
    var whereSql = "WHERE "
      + [this._filter.sql(meta, null, args, this)].concat(additionalWhereSqls).join(' AND ');
  ...
  persistence.DbQueryCollection.prototype.count = function (tx, callback) {
    ...
    var args = [];
    var whereSql = "WHERE " + [this._filter.sql(meta, "root", args, this)].concat(additionalWhereSqls).join(' AND ');
